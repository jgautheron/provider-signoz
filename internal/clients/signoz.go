...
import (
	...
	clusterv1beta1 "github.com/jgautheron/provider-signoz/apis/cluster/v1beta1"
	namespacedv1beta1 "github.com/jgautheron/provider-signoz/apis/namespaced/v1beta1"
)
...
func TerraformSetupBuilder(version, providerSource, providerVersion string) terraform.SetupFn {
	return func(ctx context.Context, client client.Client, mg resource.Managed) (terraform.Setup, error) {
		...
		// Map the ProviderConfig credentials Secret (JSON) onto the SigNoz
		// Terraform provider's config schema. `access_token` is required;
		// `endpoint` and the HTTP knobs are optional.
		ps.Configuration = map[string]any{}
		for _, k := range []string{"access_token", "endpoint", "http_timeout", "http_max_retry"} {
